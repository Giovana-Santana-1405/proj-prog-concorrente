...
def main():
    ...
    # CONFIGURAÇÃO ALTERADA PARA 4 THREADS
    qtd_threads = 4 
    ...
    with open("US_Accidents_March23.csv", "r", encoding="utf-8") as arquivo:
        ...
        with ProcessPoolExecutor(max_workers=qtd_threads) as executor:
            ...
            # Aumentamos o lote para que cada uma das 4 threads pegue 100 mil linhas por vez
            TAMANHO_LOTE = 400000  
            ...
            for linha in leitor:
                ...
                # Quando o lote acumular linhas suficientes para as 4 threads
                if len(linhas_temporarias) >= TAMANHO_LOTE:
                    lotes_enviados += 1
                    
                    # Calcula o tamanho exato de cada fatia (1/4 do lote)
                    ...
                    # Divide o lote em 4 partes e dispara as 4 threads
